...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Automatically create RDFS definitions from their SHACL counterparts.')
    parser.add_argument('-f', '--files', type=str,
                        help='Files containing the shacl definitions', dest='file', required=True, nargs='+')
    ...
    rdfsGraph.namespace_manager.bind('pht', rdflib.URIRef(
        'https://github.com/LaurenzNeumann/PHTMetadata#'))
    ...
    for s, p, classuri in graph.triples((None, SH.targetClass, None)):
        ...
        for s2, p2, prop in graph.triples((s, SH.property, None)):
            ...
            target = next(graph.triples(
                (prop, SH["class"], None)), ["", "", None])[2]
